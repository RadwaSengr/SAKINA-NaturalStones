import { BadgeCheck, Home, MessageCircle, PackageCheck } from "lucide-react";
...
export default function OrderConfirmed() {
  ...
  const whatsappMessage = isArabic
    ? `السلام عليكم سكينة، أود تأكيد ومتابعة طلبي رقم: ${reference}`
    : `Hello SAKINA, I would like to confirm my order reference: ${reference}`;
    ...
    <section className="order-confirmed-card">
      ...
      <div className="confirmation-actions">
        <a
          ...
          className="action-button confirmation-whatsapp-btn"
          style={{ background: "#1f6f50", color: "#fff", display: "inline-flex", alignItems: "center", gap: "0.5rem" }}
        >
          <MessageCircle size={16} /> {copy.whatsapp}
        </a>
        <Link href="/" className="action-button"><Home size={16} /> {copy.home}</Link>
        <Link href="/collection" className="confirmation-text-link">{copy.cabinet}</Link>
      </div>
    </section>
  </main>;
}
